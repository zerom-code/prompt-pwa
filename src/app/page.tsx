...
type ApiConfig = { apiKey: string; model: string };
...
function SettingsModal({ config, close, save, disconnect }: { config: ApiConfig; close: () => void; save: (next: ApiConfig) => void; disconnect: () => void }) {
  const [apiKey, setApiKey] = useState(config.apiKey);
  const [model, setModel] = useState(config.model);
  const [showKey, setShowKey] = useState(false);
  return <Modal title="Подключение OpenAI" close={close}><p className="modal-description">Используйте свой API‑ключ для генерации через Responses API прямо из Forma.</p>
    <label className="field-label" htmlFor="openai-model">Модель</label><input className="text-field" id="openai-model" value={model} onChange={e => setModel(e.target.value)} placeholder="gpt-5.6-luna" autoComplete="off" spellCheck={false} />
    <label className="field-label" htmlFor="openai-key">API‑ключ</label><div className="key-input"><input className="text-field" id="openai-key" type={showKey ? 'text' : 'password'} value={apiKey} onChange={e => setApiKey(e.target.value)} placeholder="sk-…" autoComplete="off" spellCheck={false} /><button type="button" onClick={() => setShowKey(!showKey)} aria-label={showKey ? 'Скрыть API-ключ' : 'Показать API-ключ'}>{showKey ? <EyeOff size={16} /> : <Eye size={16} />}</button></div>
    <div className="privacy-note"><ShieldCheck size={20} /><p><strong>Прямое подключение — осознанный компромисс.</strong> Ключ доступен коду открытой страницы и виден в сетевом запросе браузера. Forma держит его только в памяти этой вкладки: не сохраняет в localStorage, историю, файлы или GitHub. После перезагрузки ключ нужно ввести заново.</p></div>
    <div className="settings-links"><a href="https://platform.openai.com/api-keys" target="_blank" rel="noreferrer">Создать или отозвать ключ <ArrowUpRight size={14} /></a><span>Рекомендуется отдельный ключ с лимитом расходов.</span></div>
    <div className="settings-actions">{config.apiKey && <button className="secondary" type="button" onClick={disconnect}>Отключить</button>}<button className="primary" type="button" onClick={() => save({ apiKey: apiKey.trim(), model: model.trim() || 'gpt-5.6-luna' })} disabled={!apiKey.trim()}>Сохранить в этой вкладке <Check size={17} /></button></div>
  </Modal>;
}
...
export default function Home() {
  ...
  const [apiConfig, setApiConfig] = useState<ApiConfig>({ apiKey: '', model: 'gpt-5.6-luna' });
  ...
  useEffect(() => {
    ...
    const hydrateTimer = window.setTimeout(() => {
      try {
        ...
        const savedModel = localStorage.getItem('forma_openai_model');
        if (savedModel) setApiConfig(config => ({ ...config, model: savedModel }));
    ...
    const networkTimer = window.setTimeout(update, 0); window.addEventListener('online', update); window.addEventListener('offline', update);
    return () => { window.clearTimeout(hydrateTimer); window.clearTimeout(networkTimer); window.removeEventListener('online', update); window.removeEventListener('offline', update); navigator.serviceWorker?.removeEventListener('message', ready); };
  }, []);
  ...
  function applyTemplate(t: typeof templates[number]) { newPrompt(t.mode); updateInput(t.input); setOptions(o => ({ ...o, style: t.style, format: t.title.includes('Портрет') ? '4:5' : '16:9' })); setModal(null); window.scrollTo({ top: 0, behavior: 'smooth' }); }
  function saveApiConfig(next: ApiConfig) { setApiConfig(next); try { localStorage.setItem('forma_openai_model', next.model); } catch {} setModal(null); setToast('OpenAI подключён до перезагрузки страницы'); }
  function disconnectApi() { setApiConfig(config => ({ ...config, apiKey: '' })); setModal(null); setToast('API‑ключ удалён из памяти вкладки'); }
  async function generate() {
    if (loading) return;
    if (!input.trim()) { setError('Опишите вашу идею — хотя бы в нескольких словах.'); textarea.current?.focus(); return; }
    if (mode === 'reference' && !image) { setError('Сначала загрузите фото-референс.'); return; }
    if (mode === 'reference' && !apiConfig.apiKey) { setError('Для анализа референса подключите свой API‑ключ OpenAI.'); setModal('settings'); return; }
    if (apiConfig.apiKey && offline) { setError('Для запроса к OpenAI нужно подключение к интернету. Локальный шаблон по-прежнему доступен после отключения API.'); return; }
    setLoading(true); setError('');
    try {
      const previousPrompt = refining ? current?.output : undefined;
      const output = apiConfig.apiKey ? await generateWithOpenAI({ apiKey: apiConfig.apiKey, model: apiConfig.model, request: input.trim(), mode, options, previousPrompt, imageDataUrl: image || undefined }) : demoPrompt(input.trim(), mode, options, previousPrompt);
      const data: Entry = { id: crypto.randomUUID(), title: input.trim().slice(0, 80), input: input.trim(), output, mode, model: apiConfig.apiKey ? apiConfig.model : 'Локальный шаблон', options: { ...options }, favorite: false, demo: !apiConfig.apiKey, createdAt: new Date().toISOString() };
  ...
  return <div className="app-shell">
        ...
        {view === 'studio' ? <>
          ...
          <div className="composer-grid">
            ...
            <aside className="parameters-card"><div className="card-heading"><SlidersHorizontal size={18} /><h2>Тонкая настройка</h2><span className="step-badge">02</span></div><div className="parameter-field"><label htmlFor="language"><Globe2 size={14} />Язык промпта</label><div className="select-wrap"><select id="language" value={options.language} onChange={e => setOptions({ ...options, language: e.target.value })}><option>English</option><option>Русский</option></select><ChevronDown size={14} /></div><span className="field-hint">Английский — универсальный выбор для ИИ</span></div>
              ...
              <div className="parameter-divider" /><div className="model-label">РЕЖИМ ГЕНЕРАЦИИ</div><button className="model-select" onClick={() => setModal('settings')}><span className="model-icon"><Command size={22} /></span><span><strong>{apiConfig.apiKey ? apiConfig.model : 'Локальный шаблон'}</strong><small>{apiConfig.apiKey ? 'OpenAI Responses API' : 'Без API‑ключа'}</small></span><ChevronDown size={15} /></button><div className="model-note"><span className={apiConfig.apiKey ? 'green-dot' : 'purple-dot'} />{apiConfig.apiKey ? 'Прямое подключение · ключ в памяти вкладки' : 'Работает локально и без API‑ключа'}</div>
            </aside>
          </div>
    ...
    {modal === 'settings' && <SettingsModal config={apiConfig} close={() => setModal(null)} save={saveApiConfig} disconnect={disconnectApi} />}
